...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare('UPDATE transactions SET type = ?, affects_balance = ?, amount = ?, description = ?, date = ? WHERE id = ?');
        $stmt->execute([$type, $affects_balance, $amount, $description, $date, $id]);
        redirect_with_message(public_url('user-transactions') . '?user_id=' . $user_id, 'success', 'İşlem güncellendi');
