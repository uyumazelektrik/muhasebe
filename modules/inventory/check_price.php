        ...
        <div class="w-full max-w-2xl bg-white dark:bg-card-dark rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-8">
            <div class="relative mb-8">
                ...
                <input type="text" id="searchInput" autofocus 
                       class="w-full h-16 pl-14 pr-32 rounded-2xl bg-slate-50 dark:bg-input-dark border-2 border-slate-100 dark:border-slate-700 text-xl font-bold focus:border-primary transition-all"
                       placeholder="Barkod okutun veya isim yazın...">
                
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 gap-2">
                    <label for="fileInput" class="w-12 h-12 flex items-center justify-center rounded-xl bg-primary text-white hover:bg-primary-dark transition-colors shadow-lg shadow-primary/20 cursor-pointer">
            ...
            <div id="aiSearchArea" class="hidden mb-8 p-6 bg-primary/5 border border-primary/20 rounded-3xl animate-in fade-in zoom-in duration-300">
                <div class="flex flex-col items-center gap-4 text-center">
                    <div class="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center text-primary relative">
                        <span class="material-symbols-outlined text-[32px] spin">neurology</span>
...
(function() {
    ...
    const GEMINI_API_URL = '<?php echo public_url('api/gemini-search'); ?>';
    const SEARCH_STOCK_API_URL = '<?php echo public_url('api/search-stock'); ?>';
    ...
    function showMultipleResults(items, isAi = false) {
        ...
        items.forEach(item => {
            ...
            const imgHtml = item.gorsel 
                ? `<img src="${item.gorsel}" class="w-12 h-12 object-cover rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">`
