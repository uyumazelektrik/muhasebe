...
<script>
    async function openPayrollDetail(userId, month, year) {
        ...
        try {
            const response = await fetch(`<?php echo public_url('api/get-user-payroll'); ?>?user_id=${userId}&month=${month}&year=${year}`);
            ...
            if (data.details.length === 0) {
                ...
                data.details.forEach(item => {
                    ...
                    // İzinlerde saat göstermeyip sadece durumu yazabiliriz, ya da ikisini de.
                    // Eğer giriş çıkış yoksa (boşsa) status badge gösterelim.
                    if ((!item.clock_in || item.clock_in === '00:00') && (!item.clock_out || item.clock_out === '00:00')) {
                        timeDisplay = `<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${statusInfo.class}">${statusInfo.label}</span>`;
                         ...
                         // Saat var ama yanında durumunu da belirtelim (Özellikle Geç Kalma vs için)
