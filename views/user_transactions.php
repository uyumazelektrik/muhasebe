...
<header class="w-full bg-background-light dark:bg-background-dark border-b border-slate-200 dark:border-slate-800/50 pt-6 pb-2 px-4 sm:px-8 shrink-0">
    <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="<?php echo public_url('users'); ?>" class="grow sm:grow-0 px-3 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-gray-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors font-medium text-xs sm:text-sm text-center">
                Geri Dön
            </a>
            <a href="<?php echo public_url('api/export-transactions-csv') . '?user_id=' . $user['id'] . ($month ? '&month='.$month.'&year='.$year : ''); ?>" class="grow sm:grow-0 flex items-center justify-center gap-2 px-3 py-2 bg-white dark:bg-card-dark border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors font-medium text-xs sm:text-sm">
                <span class="material-symbols-outlined text-[16px] sm:text-[18px]">file_download</span>
                CSV
            </a>
...
<main class="flex-1 p-4 sm:px-8 w-full min-w-0">
    ...
    <div class="mb-4">
        <?php if ($_GET['status'] === 'success'): ?>
            <div class="px-4 py-3 rounded-lg bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300">
                <?php echo htmlspecialchars($_GET['message'] ?? 'İşlem başarılı'); ?>
            </div>
        <?php else: ?>
            <div class="px-4 py-3 rounded-lg bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300">
                <?php echo htmlspecialchars($_GET['message'] ?? 'İşlem başarısız'); ?>
            </div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="mb-6">
        <form method="GET" action="<?php echo public_url('user-transactions'); ?>" class="flex flex-wrap items-center gap-2 sm:gap-3 bg-slate-50 dark:bg-slate-800/50 p-2 sm:p-3 rounded-xl border border-slate-200 dark:border-slate-700">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6">
        ...
        <div class="p-4 rounded-xl bg-white dark:bg-card-dark border border-slate-200 dark:border-slate-800 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="text-xl font-bold text-slate-900 dark:text-white"><?php echo $remainingLeave; ?> / <?php echo $user['annual_leave_days']; ?></h4>
                    <span class="text-sm text-[#9da6b9]">Kalan İzin (Gün)</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden w-full">
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm">
                                ...
                                if ($txn['affects_balance'] == 1) {
                                    $harcama = ['Harcama', 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300'];
                                } else if ($txn['affects_balance'] == -1) {
                                    $harcama = ['Harcama', 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300'];
                                } else {
                                    $harcama = ['Harcama', 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300'];
                                }
                            ...
                            <?php 
                                if($txn["affects_balance"]==1){ $symbol =  "+"; $color="text-green-600 dark:text-green-400"; } else if($txn["affects_balance"]==-1){ $symbol = "-"; $color="text-red-600 dark:text-red-400"; } else { $symbol = ""; $color="text-slate-600 dark:text-gray-400"; }    
                            ?>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form method="POST" action="<?php echo public_url('api/delete-transaction'); ?>" onsubmit="return confirm('Bu işlemi silmek istediğinize emin misiniz?');">
        ...
        <?php 
            $totalPages = ceil(($totalTransactions ?? 0) / ($limit ?? 20));
        ?>
        ...
        <div class="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <div class="flex gap-2">
                <?php if ($page > 1): ?>
                    <a href="<?php echo public_url('user-transactions') . '?user_id=' . $user['id'] . ($month? '&month='.$month.'&year='.$year:'') . '&page=' . ($page-1); ?>" class="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm text-slate-600 dark:text-slate-300">&laquo; Önceki</a>
                <?php endif; ?>
                <?php if ($page < $totalPages): ?>
                    <a href="<?php echo public_url('user-transactions') . '?user_id=' . $user['id'] . ($month? '&month='.$month.'&year='.$year:'') . '&page=' . ($page+1); ?>" class="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm text-slate-600 dark:text-slate-300">Sonraki &raquo;</a>
                <?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
...
<div id="addTransactionModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm">
    <div class="bg-white dark:bg-card-dark p-6 rounded-xl w-full max-w-md shadow-2xl border border-gray-200 dark:border-border-dark">
        <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-4">Yeni İşlem Ekle</h3>
        <form method="POST" action="<?php echo public_url('api/add-transaction'); ?>" class="flex flex-col gap-4">
            <input type="hidden" name="user_id" value="<?php echo $user['id']; ?>">
            
            <label class="block">
                <span class="text-sm font-medium text-slate-700 dark:text-gray-300">İşlem Türü</span>
                <select name="type" id="add_txn_type" required class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary">
            ...
            <label class="block">
                ...
                <input type="number" step="0.01" name="amount" required class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary">
            </label>

            <label class="block">
                <span class="text-sm font-medium text-slate-700 dark:text-gray-300">Tarih</span>
                <input type="date" name="date" required value="<?php echo date('Y-m-d'); ?>" class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary">
            </label>
            ...
            <label class="block">
                ...
                <textarea name="description" rows="3" class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary"></textarea>
            </label>

            <div class="flex justify-end gap-3 mt-4">
                <button type="button" onclick="document.getElementById('addTransactionModal').classList.add('hidden')" class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-slate-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">İptal</button>
...
<div id="editTransactionModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm">
    <div class="bg-white dark:bg-card-dark p-6 rounded-xl w-full max-w-md shadow-2xl border border-gray-200 dark:border-border-dark">
        <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-4">İşlemi Düzenle</h3>
        <form method="POST" action="<?php echo public_url('api/edit-transaction'); ?>" class="flex flex-col gap-4">
            <input type="hidden" name="id" id="edit_txn_id">
            <input type="hidden" name="user_id" value="<?php echo $user['id']; ?>">
            
            <label class="block">
                <span class="text-sm font-medium text-slate-700 dark:text-gray-300">İşlem Türü</span>
                <select name="type" id="edit_txn_type" required class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary">
                    <option value="payment">Ödeme (Maaş)</option>
                    <option value="advance">Avans</option>
                    <option value="expense">Harcama</option>
                </select>
            ...
            <label class="block">
                <span class="text-sm font-medium text-slate-700 dark:text-gray-300">Tutar (₺)</span>
                <input type="number" step="0.01" name="amount" id="edit_txn_amount" required class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary">
            </label>

            <label class="block">
                <span class="text-sm font-medium text-slate-700 dark:text-gray-300">Tarih</span>
                <input type="date" name="date" id="edit_txn_date" required class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary">
            </label>
            ...
            <label class="block">
                <span class="text-sm font-medium text-slate-700 dark:text-gray-300">Açıklama</span>
                <textarea name="description" id="edit_txn_description" rows="3" class="mt-1 block w-full rounded-lg bg-gray-50 dark:bg-background-dark border-gray-300 dark:border-gray-600 text-slate-900 dark:text-white focus:ring-primary focus:border-primary"></textarea>
            </label>

            <div class="flex justify-end gap-3 mt-4">
                <button type="button" onclick="document.getElementById('editTransactionModal').classList.add('hidden')" class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-slate-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">İptal</button>
...
<script>
    function openEditTransactionModal(txn) {
        ...
        // Set affects_balance radio button
    ...
    function setEditAffectsByType() {
        const type = document.getElementById('edit_txn_type').value;
        const base = document.getElementById('editTransactionModal');
        ['-1','0','1'].forEach(v => {
            const el = base.querySelector('input[name="affects_balance"][value="'+v+'"]');
            if (el) el.checked = false;
        });
        if (type === 'advance') {
            base.querySelector('input[name="affects_balance"][value="-1"]').checked = true;
        } else if (type === 'payment' || type === 'salary_accrual') {
            base.querySelector('input[name="affects_balance"][value="0"]').checked = true;
        } else if (type === 'expense') {
            base.querySelector('input[name="affects_balance"][value="1"]').checked = true;
        }
    }
    document.getElementById('edit_txn_type').addEventListener('change', setEditAffectsByType);
</script>
